    ...
    public class StatsRepository : IStatsRepository
    {
        ...
        public MinerWorkerPerformanceStats[] PagePoolMinersByHashrate(IDbConnection con, string poolId, DateTime from, int page, int pageSize)
        {
            ...
            var query = "WITH tmp AS " +
                        ...
                        "		ms.hashrate,  " +
                        "		ms.sharespersecond,  " +
                        "		ROW_NUMBER() OVER(PARTITION BY ms.miner ORDER BY ms.created DESC) AS rk  " +
                        "	FROM (SELECT miner, SUM(hashrate) AS hashrate, SUM(sharespersecond) AS sharespersecond, created " +
                        "       FROM minerstats " +
