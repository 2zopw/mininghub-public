...
namespace MiningHub.Core.Persistence.Postgres.Repositories
{
    public class ShareRepository : IShareRepository
    {
        ...
        public DateTime GetLastShare(IDbConnection con, string poolId, string miner, string worker)
        {
            ...
            var query = "SELECT MAX(created) AS lastshare FROM shares " +
                        "WHERE poolid = @poolId AND miner = @miner AND worker = @worker";
            return con.Query<DateTime>(query, new { poolId, miner, worker }).FirstOrDefault();
        }
    }
}
